[berain-server](https://github.com/fengfei1000/berain/tree/master/berain-server)
=======


Install (Play framework 1.2.5)[https://github.com/playframework/play]
---------------
* (download play-1.2.5.zip) [http://download.playframework.org/releases/play-1.2.5.zip], and unzip play-1.2.5.zip -d /path/to/play-1.2.5
* Add PLAY_HOME
** Windows PLAY_HOME=c:\path\to\play PATH=%PLAY_HOME%;...
** Linux  
***       PLAY_HOME=/path/to/play 
***       PATH=$PLAY_HOME:$PATH
***       export PLAY_HOME PATH
*cd   /path/to/berain-server
*play run or play start
* Go to http://localhost:8021 and you¡¯ll see the login page. default user name is "admin", password is "password"

[snapshot.jpg]