...
public class Berain extends Controller {
	...
	public static void create(String id, String value) {
		...
		try {
			...
			List<RainModel> models = RainModel.find("path=?", parentPath).fetch();
			String pid = "0";
			if (models != null && models.size() >= 1) {
	...
	public static void copy(String originalId, String newid) {
		...
		try {
			List<RainModel> models = RainModel.find("path=?", originalId).fetch();
			RainModel orig = null;
			if (models != null && models.size() == 1) {
				orig = models.get(0);
				orig.path = newid;
	...
	public static void removeWatchable(String clientId, String id, int type) {
		try {
			container.removeWatchableEvent(clientId, new WatchableEvent(type, id));
			renderJSON(new BerainResult(Success, true));
	...
	public static void listChangedNodes(
			String clientId,
			List<String> paths,
			List<Integer> types) {
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				WatchableEvent event = container.varifyWatchableEvent(
						clientId,
						path,
						eventType);
			}

			Map<String, Set<WatchedEvent>> edata = container.getAllWatchedEvents(clientId);
			if (edata != null)
				data.putAll(edata);
			renderJSON(new BerainResult(Success, data));
		} catch (Throwable e) {
	...
	public static void dump(String clientId) {
		if (clientId == null || "".equals(clientId)) {
			...
			renderJSON(new BerainResult(Success, container.getWatcheds().get(clientId)));
		}
